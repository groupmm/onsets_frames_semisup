...
def evaluate(data, model, onset_threshold=0.5, frame_threshold=0.5, save_path=None, return_filenames=False):
    ...
    for label in data:
        filenames.append(os.path.basename(label['path']))
        pred, losses = model.run_on_batch(label)

        for key in ['onset', 'offset', 'frame']:
            all_preds[key].append(pred[key].cpu().detach())
            all_labels[key].append(label[key].cpu().detach())
        ...
        metrics['metric/frame/average_precision_score'].append(sk_metrics.average_precision_score(label['frame'].cpu().flatten(), pred['frame'].detach().cpu().flatten()))

        if save_path is not None:
            os.makedirs(save_path, exist_ok=True)
            label_path = os.path.join(save_path, os.path.basename(label['path']) + '.label.png')
            save_pianoroll(label_path, label['onset'], label['frame'], onset_threshold=onset_threshold, frame_threshold=frame_threshold)
            pred_path = os.path.join(save_path, os.path.basename(label['path']) + '.pred.png')
            save_pianoroll(pred_path, pred['onset'], pred['frame'], onset_threshold=onset_threshold, frame_threshold=frame_threshold)
            midi_path = os.path.join(save_path, os.path.basename(label['path']) + '.pred.mid')
            save_midi(midi_path, p_est, i_est, v_est)

    if return_filenames:
        return metrics, filenames
    else:
        return metrics
